...
class Shape:
    ...
    def measure(self):
        raise NotImplementedError('This method should be implmented by subclasses.')
    """
...
class Triangle(self):
    ...
    def measure(self): 
        a = math.sqrt((self.p1[0] - self.p2[0]) ** 2 + (self.p1[1] - self.p2[1]) ** 2)
        b = math.sqrt((self.p1[0] - self.p3[0]) ** 2 + (self.p1[1] - self.p3[1]) ** 2)
        c = math.sqrt((self.p2[0] - self.p3[0]) ** 2 + (self.p2[1] - self.p3[1]) ** 2)
        s = (a + b + c)/2
